...
export class PharmacyService implements OnModuleInit {
  ...
  async getDashboard(organizationId: string, branchCode: string) {
    ...
    for (const b of batches) {
      ...
      if (exp <= in30Str && b.quantity > 0) {
        expiringCount += 1;
        const med = medicines.find((m) => m.id === b.medicineId);
        if (med) {
          alerts.push({
            type: exp <= today ? "expiry" : "near_expiry",
            severity: exp <= today ? "danger" : "warning",
            message: `${med.name} batch expires ${exp}`,
            medicineId: med.id,
          });
        }
      }
  ...
  async createSale(organizationId: string, input: CreatePharmacySale) {
    const branch = await this.resolveBranch(organizationId, input.branchCode);
    const invoiceNumber = `INV-${Date.now().toString().slice(-6)}`;

    let subtotal = 0;
    let tax = 0;
    const lineData: { medicineId: string; qty: number; unitPrice: number; lineTotal: number; name: string }[] = [];
    ...
    for (const line of input.lines) {
      ...
      if (med.currentStock < line.qty) throw new BadRequestException(`Insufficient stock for ${med.name}`);
      const lineTotal = med.sellingPricePkr * line.qty;
    ...
    for (const line of lineData) {
      ...
      await this.deductMedicineStock(organizationId, branch.id, line.medicineId, line.qty);
    }
    ...
    if (input.patientId) {
      ...
      if (patient) {
        await this.db
          ...
          .set({ loyaltyPoints: patient.loyaltyPoints + Math.floor(total / 100) })
