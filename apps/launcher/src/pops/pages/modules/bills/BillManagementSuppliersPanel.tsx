...
import { ModuleCountBadge, ModuleFilterBar, ModuleSegmentedControl } from "../../../ui/ModuleToolbar";
...
export function BillManagementSuppliersPanel(): JSX.Element {
  ...
  if (!branch?.code) {
    return <p className="text-sm text-slate-500">Select a branch to manage suppliers.</p>;
  }
  if (query.isLoading) return <InventoryLoading />;
  if (query.isError) return <InventoryError message={(query.error as Error).message} />;

  function confirmDeleteSupplier(supplier: Supplier): void {
    if (
      !confirm(
        `Permanently delete supplier "${supplier.name}"?\n\nThis cannot be undone. Purchase history linked to this supplier will be removed.`,
      )
    ) {
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-600 dark:text-slate-400">
        Manage supplier contacts, payment terms, and purchase relationships for this branch.
      </p>
      ...
      {canManage ? (
        ...
        >
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <label className="flex flex-col gap-1">
              ...
              <input className={inputClass} type="date" value={form.onboardedDate} onChange={(e) => setForm({ ...form, onboardedDate: e.target.value })} />
            </label>
        ...
        <p className="text-xs text-slate-500">You need pops.inventory.manage permission to add or edit suppliers.</p>
      )}
      ...
      <ModuleFilterBar>
        ...
        {filterDate ? (
          <button type="button" className={`self-end text-xs ${linkActionClass}`} onClick={() => setFilterDate("")}>
            Clear date
          </button>
        ) : null}
                ...
                ? [
                    {
                      ...
                      render: (r: Supplier) => (
                        <div className="flex gap-2">
                          ...
                          <button type="button" className={`text-xs ${linkDangerClass}`} onClick={() => confirmDeleteSupplier(r)}>
                            Delete
                          </button>
                        </div>
                      ),
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900/40">
          ...
          <ul className="mt-3 space-y-2 text-sm text-slate-600 dark:text-slate-300">
            ...
            <li className="flex justify-between">
              ...
              <span className={accentValueClass}>{formatPkr(filteredSuppliers.reduce((s, x) => s + x.totalPurchases, 0))}</span>
            </li>
          </ul>
        </div>
      </div>
