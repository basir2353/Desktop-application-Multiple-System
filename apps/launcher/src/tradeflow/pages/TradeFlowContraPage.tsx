          ...
          {(rows.data ?? []).map((r) => (
            <tr key={r.id} className="border-t border-slate-100 dark:border-slate-800">
              ...
              <td className="px-3 py-2">
                <button type="button" className="text-amber-800 dark:text-amber-300" onClick={() => printTradeFlowSlip(`Contra ${r.entryNo}`, [["From", r.fromAccountName], ["To", r.toAccountName], ["Amount", formatPkr(r.amountPkr)], ["Narration", r.narration ?? ""], ["Date", new Date(r.createdAt).toLocaleString()]])}>Print</button>
              </td>
            </tr>
          ))}
        </tbody>
