...
export async function loadMobilePrinterSettings(): Promise<MobilePrinterSettings> {
  try {
    ...
    if (!raw) return { ...DEFAULT_MOBILE_PRINTER_SETTINGS, kitchenPrinters: [...DEFAULT_MOBILE_PRINTER_SETTINGS.kitchenPrinters] };
    return normalize(JSON.parse(raw) as Partial<MobilePrinterSettings>);
  } catch {
    return { ...DEFAULT_MOBILE_PRINTER_SETTINGS, kitchenPrinters: [...DEFAULT_MOBILE_PRINTER_SETTINGS.kitchenPrinters] };
  }
}
