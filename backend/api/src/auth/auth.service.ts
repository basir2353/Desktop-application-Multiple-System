...
export class AuthService implements OnModuleInit {
  ...
  async seedIfEmpty(): Promise<void> {
    ...
    // Demo tenant seeding is opt-in — portal shows real businesses by default.
    if (this.config.get<string>("SEED_DEMO_DATA") !== "true") return;
  ...
  /** Soft-delete seeded demo businesses so the control plane only lists real clients. */
  private async hideDemoData(): Promise<void> {
    if (this.config.get<string>("SEED_DEMO_DATA") === "true") return;
    ...
    await this.db
      ...
      .where(
        and(
          ...
          sql`(
            lower(coalesce(${organizations.licencePlan}, '')) = 'demo'
            OR lower(${organizations.name}) like 'demo %'
            OR lower(${organizations.name}) = 'demo organization'
            OR lower(${organizations.name}) = 'demo restaurant'
          )`,
        ),
      );
  }
  ...
  async login(email: string, password: string) {
    ...
    if (org.licenceExpiresAt && org.licenceExpiresAt.getTime() < Date.now()) {
      throw new UnauthorizedException(
        "This business licence has expired. Contact the platform administrator.",
      );
    }
